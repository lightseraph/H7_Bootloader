...
u8 SDMMC_DATA_BUFFER[512] __attribute__((aligned(4)));
...
SD_Error SDEnWideBus(u8 enx)
{
	...
	if (enx)
		...
		arg = 0X00;
	if (SDMMC1->RESP1 & SD_CARD_LOCKED)
...
u8 SD_ReadDisk(u8 *buf, u32 sector, u32 cnt)
{
	...
	if ((u32)buf % 4 != 0)
	{
		...
		sta = SD_ReadBlocks(buf, lsector, 512, cnt); //单个/多个sector
	return sta;
}
//写SD卡
// buf:写数据缓存区
...
u8 SD_WriteDisk(u8 *buf, u32 sector, u32 cnt)
{
	...
	if ((u32)buf % 4 != 0)
	{
		...
		sta = SD_WriteBlocks(buf, lsector, 512, cnt); //单个/多个sector
	return sta;
}
